...
parser.add_argument('--log_dir', default='C:/PSR-GAT/model/new', help='Log dir')
...
parser.add_argument('--max_epoch', type=int, default=60, help='Epoch to run [default: 80]')
parser.add_argument('--batch_size', type=int, default=32, help='Batch Size during training [default: 24]')
parser.add_argument('--learning_rate', type=float, default=0.0003)
#parser.add_argument('--dataset', default='C:/PSR-GAT/model/data/Patches_noHole_and_collected.h5')
parser.add_argument('--dataset', default='C:/PSR-GAT/PU1K/train/pu1k.h5')
parser.add_argument('--gan', default=False, action='store_true')
parser.add_argument('--model_path', type=int, default=False, help='The num of epoch to restore the models from')
...
parser.add_argument('--FWWD', default=True, action='store_true',
...
parser.add_argument('--num_workers_each_gpu', type=int, default=2, help='[default: 4]')
...
'''
torch.cuda.set_device(0)# dauflt=0
...
def train(assign_model_path=None, bn_decay=0.95):
    ...
    g_model = MODEL_GEN.GenModel(use_normal=False, use_bn=False, use_ibn=False, bn_decay=bn_decay, up_ratio=UP_RATIO,
                                 multi_gpus=False, bachnum=BATCH_SIZE)
    g_model.train()
    ...
    for epoch in tqdm(range(restore_epoch, MAX_EPOCH + 1), ncols=55):
        ...
        for i_batch, sample_batched in enumerate(tqdm(dloader)):
            ...
            if not FLAGS.FWWD:
                ...
                WD_loss = torch.FloatTensor([0.]).cuda()
            else:
                '''
            ...
            scaler.scale(pre_gen_loss).backward()
            scaler.step(g_fc_optimizer)
            ...
            log_writer.add_scalar('gen_loss/train', pre_gen_loss.cpu().detach(), n_iter)
...
def prediction_whole_model(use_normal=False, this_scale=4):
    ...
    if multi_gpus:
        g_model = torch.nn.DataParallel(g_model, device_ids=gpu_ids).cuda()
    else:
        g_model = g_model.cuda()
    ...
    for i, item in enumerate(samples):
        ...
        with torch.no_grad():
            input_torch = torch.from_numpy(input).type(torch.cuda.FloatTensor).detach().cuda()
            beg = time.time()
