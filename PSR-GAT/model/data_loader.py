...
def jitter_perturbation_point_cloud(batch_data, sigma=0.005, clip=0.02): # noise,orginal sigma = 0.005
...
class LoaderDataset(data.Dataset):
    def __init__(self, dataset, batch_size, use_norm, gpu_ids, transforms=None, train=True, refinenet=False):
        ...
        if not refinenet:
            self.input_data, self.gt_data, self.radius_data = load_patch_data(self.dataset, skip_rate=1,
                                                                                 norm=use_norm)
        else:
